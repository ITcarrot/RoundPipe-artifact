#!/usr/bin/env python3
"""Plot training throughput comparison on 8x A800."""
...
FIG_PATH = os.path.join(SCRIPT_DIR, 'a800_throughput.pdf')
...
# Row indices (0-based) and display names for A800 frameworks
FRAMEWORK_ROWS_A800 = [
    (9,  'ZeRO-2'),
    (11, 'FSDP'),
    (10, 'ZeRO-Infinity'),
...
def main():
    ...
    frameworks = []
    data = {}
    for row_idx, name in FRAMEWORK_ROWS_A800:
        row = rows[row_idx]
        vals = [parse_val(row[c]) for c in range(2, 7)]
    ...
    for i, fw in enumerate(frameworks):
        ...
        for j, v in enumerate(vals):
            if v is None:
                tag = rows[FRAMEWORK_ROWS_A800[i][0]][2 + j].strip()
                ax.text(x[j] + offset, 200, tag,
    ...
    ax.set_title('Training Throughput on 8×A800 SXM', fontsize=13)
