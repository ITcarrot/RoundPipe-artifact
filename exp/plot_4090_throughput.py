#!/usr/bin/env python3
"""Plot training throughput comparison on 8x RTX 4090."""
...
FIG_PATH = os.path.join(SCRIPT_DIR, '4090_throughput.pdf')
...
def parse_val(s):
    """Parse a throughput value; return None for OOM/N/A."""
    s = s.strip()
    if s in ('OOM', 'N/A', ''):
        return None
    return float(s)
...
def main():
    ...
    for i, fw in enumerate(frameworks):
        ...
        for j, v in enumerate(vals):
            if v is None:
                tag = rows[FRAMEWORK_ROWS_4090[i][0]][2 + j].strip()
    ...
    ax.set_title('Training Throughput on 8×RTX 4090', fontsize=13)
